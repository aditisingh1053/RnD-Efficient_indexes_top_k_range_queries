...
def visualize_points(points, weights, selected_indices, distribution_name,force):
    ...
    plt.title(f'{distribution_name} Distribution, lambda = {force}')
    plt.legend()
    os.makedirs("plots_mmd", exist_ok=True)
    plt.savefig(f"plots_mmd/{distribution_name}_{force}_MSD.png", dpi=300)
    plt.close()
