...
def compute_f(points, lam):
    ...
    distsum = 0.0
    for i in range(n):
        distsum += np.linalg.norm(coords[i] - coords[i+1:], axis=1).sum()
    return distsum + lam * weights.sum()
...
EOF
    ...
    if [[ "$status" == "PASS" ]]; then
        ...
        echo -e "Expected f(S): $expected_val"
        echo -e "Actual f(S):   $actual_val"
...
rm -f temp_actual.txt MSD_explicit
